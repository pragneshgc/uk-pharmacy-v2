        ...
        <div v-if="visible" class="esa-modal" id="productEditor" tabindex="-1" role="dialog"
            ...
            <div class="modal" role="document">
                ...
                <div v-if="productCodeId && !pricingId && !editing" class="modal-header"
                    style="background: transparent; box-shadow: none;border-bottom: 1px solid gainsboro; display: flex; align-items: center;">
                    ...
                    <select class="ml-10 browser-default custom-select" v-model="productCodeId" name="ProductCodeID">
                        ...
                        <option v-for="option in options" :key="option.ProductCodeID" :value="option.ProductCodeID">{{
                            option.Name }}</option>
                    </select>

                    <button v-if="productCode.Status == 1" @click="deleteProduct()"
                        class="ml-10 btn btnSize02 tertiaryBtn">
                                ...
                                <table v-if="list.length > 0" id="products-table" class="table table-hover">
                                    <thead class="primary-color text-white">
                                        <th v-for="(value, key) in list[0]" :key="key"
                                            v-if="!['UnformattedPrice', 'PricingID'].includes(key)">
                                            {{ key }}
                                        </th>
                                        <th>
                                            Options
                                    ...
                                    <tbody>
                                        <tr v-for="(value) in list" :key="value.Code">
                                            <td v-for="(v, k) in value" :key="k"
                                                v-if="!['UnformattedPrice', 'PricingID'].includes(k)">
                                                {{ v }}
                                            </td>
                                            <td>
                                                ...
                                                <button v-if="value.ClientID != 'DEFAULT'"
                                                    @click="removePricing(value.PricingID)" class="table-icon">
                            ...
                            <div class="pxp-form wow fadeIn" v-else-if="type == 'product' && editing">
                                <div class="form-column">
                                    ...
                                    <div class="form-group form-group-2">
                                        ...
                                        <select class="browser-default custom-select" v-model="productCodeEdit.Fridge"
                                            name="Fridge">
                                        ...
                                        <div v-if="errors.Fridge" class="invalid-feedback d-block">{{ errors.Fridge[0]
                                            }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="form-column">
                                    ...
                                    <div class="form-group form-group-2">
                                        ...
                                        <select class="browser-default custom-select" v-model="productCodeEdit.Status"
                                            name="Status">
                                            <option value="1" selected>Active</option>
                                            <option value="0">Inactive</option>
                                            <option value="2">Discontinued</option>
                                        </select>
                                        <div v-if="errors.Status" class="invalid-feedback d-block">{{ errors.Status[0]
                                            }}</div>
                                    </div>
                ...
                <div class="modal-footer">
                    <button v-if="editing" :disabled="saving" @click="save()" type="button"
                        class="btn btnSize02 tertiaryBtn">
                        {{ saveButtonText }}
                    </button>

                    <button v-if="!editing && productCodeId" :disabled="saving" @click="addPricing()" type="button"
                        class="btn btnSize02 tertiaryBtn">
                        Add New Pricing
                    </button>

                    <button v-if="editing && productCodeId" :disabled="saving" @click="back()" type="button"
                        class="btn btnSize02 tertiaryBtn">
                        Back
                    </button>
...
export default {
    ...
    mounted() {
        this.emitter.on('label.details', this.getProductDetails);
        this.emitter.on('label.add', this.addProduct);
        this.clear();
    },
    beforeDestroy() {
