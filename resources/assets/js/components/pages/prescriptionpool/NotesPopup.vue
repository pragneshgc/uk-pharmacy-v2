    ...
    <div class="medicineDetails" style="width: 100%; padding: 0; height: 100%;">
        <ul class="tabs">
            ...
            <li title="Relates communication with perscriber and notes sent with perscription"
                ...
                <a href="javascript:;">Queried Notes
                    <span v-if="(notes.correspondence.length + notes.information.length) > 0" class="badge red">{{
                        notes.correspondence.length + notes.information.length }}</span>
                </a>
            </li>
            <li title="Relates to the current order" :class="{ 'active': activeTab == 'order' }"
                @click="activeTab = 'order'">
                <a href="javascript:;">Order Notes
                    <span v-if="(notes.other.length || (prescription.Notes != '' && prescription.Notes != null)) > 0"
                        class="badge red">
        ...
        <div class="content" v-if="activeTab == 'notes'">
            <ul v-if="notes.critical.length > 0" class="critical">
                <li class="note" :class="[note.DeletedAt != null || note.EditedAt != null ? 'deleted' : '']"
                    ...
                    <div class="note-footer"
                        ...
                        <div>
                            ...
                            <button v-if="note.UserID == userInfo.id" @click="openNote(note)"
                                class="smallTextBtn secondaryBtn" style="font-size: 12px; cursor:pointer;">Edit</button>
                        </div>
                    </div>
                            ...
                            <li class="note" v-for="edited in note.Edits" :key="edited.NoteID">
                                ...
                                <div class="note-footer" style="color:#ff8944;">
                                    <span>Edited By {{ edited.EditedName }}
                                        {{ edited.EditedSurname }}</span><span>{{ edited.EditedAt }}</span>
                                </div>
                            </li>
                        </ul>
                    </div>
            ...
            this.$swal({
                ...
                if (result.value) {
                    axios.post('/note/' + id + '/delete')
                        ...
                        .finally(() => {
                            this.emitter.emit('prescriptionpool.getnotes');
                        })
                }
            })
        },
