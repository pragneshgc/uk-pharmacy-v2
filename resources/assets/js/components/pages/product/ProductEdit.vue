    ...
    <transition name="fade">
        ...
        aria-labelledby="itemEditorLabel" aria-modal="true" >
            ...
            <div id="product-modal" class="modal" role="document">
                ...
                <div class="modal-header" v-if="productCodeId && !pricingId && !editing" style="display: flex;justify-content: space-between;align-items: center;border: none;padding-bottom: 0;">
                    <select class="ml-10 browser-default custom-select" v-model="productCodeId" name="ProductCodeID" style="font-weight:700;">
                        <option value="">SELECT</option>
                        <option v-for="option in optionsFiltered" :key="option.ProductCodeID" :value="option.ProductCodeID">
                            <b>{{ option.Name }}<span v-if="type == 'product'">  - {{ option.Code }} - ({{ option.Quantity }})</span></b>
                        </option>
                    </select>

                <div v-if="productCodeId && !pricingId && !editing" class="modal-header" style="background: transparent; box-shadow: none;border-bottom: none; display: flex; align-items: center;">
                    <button @click="editProduct(productCode)" v-if="userInfo.role > 4" class="btn btnSize02 secondaryBtn">
                        Update Product Code
                    </button>

                    <button v-if="productCode.Pack && userInfo.role > 4" title="Save product" @click="addPackProduct()" class="ml-10 btn btnSize02 secondaryBtn">
                        ADD Product To Pack
                    </button>
                    ...
                    <div class="expand-100 d-flex justify-content-between">
                        <div class="row wow fadeIn" style="width: 100%;">
                            ...
                            <div class="col-lg-12 wow fadeIn" v-if="productCodeId && !pricingId && !editing">
                                <h3 style="text-align: center;margin-top: 15px;padding-bottom: 5px;border-bottom: 3px solid rgb(209 96 96);float:left;width: 100%;">Pricing</h3> 
                                <table id="products-table" class="table table-hover" style="border-bottom: none;">
                                    <thead class="primary-color text-white">
                                        <th v-for="(value, key) in list[0]" :key="key" v-if="!['PackProductID','UnformattedPrice', 'PricingID', 'Status'].includes(key)">
                                            {{ key }}
                                        </th>
                                    ...
                                    <!-- <tbody> -->
                                        <draggable
                                            ...
                                            tag="tbody"
                                        >
                                            <tr v-for="(value) in list" :key="value.Code" :class="[productCode.Pack ? (value.Status ? 'enabled' : 'disabled') : '']">
                                                <td v-for="(v, k) in value" :key="k" v-if="!['PackProductID','UnformattedPrice', 'PricingID', 'Status'].includes(k)">
                                                    {{ v }}
                                                </td>
                                                <td v-if="productCode.Pack">
                                                    <input :class="{'unchecked': (!value.Status)}" :checked="(value.Status)" 
                                                    type="checkbox" name="checkall"> 
                                                    <label @click="changeStatus(value)" for="checkall"></label>
                                                </td>
                                                <td>
                                                    <button v-if="!productCode.Pack && userInfo.role > 4" @click="edit(value)" class="btn btn-primary table-icon">
                                                        <i class="fa fa-edit" style="color: white;"></i>
                                                    </button>

                                                    <button v-if="!productCode.Pack && value.ClientID != 'DEFAULT' && userInfo.role > 4" @click="removePricing(value)" class="btn btn-primary table-icon">
                                                        <i class="fa fa-trash" style="color: white;"></i>
                                                    </button>          

                                                    <button v-if="productCode.Pack && userInfo.role > 4" @click="removeProduct(value.PackProductID)" class="btn btn-primary table-icon">
                                                        <i class="fa fa-trash" style="color: white;"></i>
                                                    </button>
                                                </td>
                                                <!-- <td>{{ value.Status }}</td> -->
                                        ...
                                        <tr v-if="addedProduct && productCode.Pack">
                                            ...
                                            <td>
                                                <select class="ml-10 browser-default custom-select" v-model="addedProduct.Description" name="Description">
                                            ...
                                            <td>
                                                <div class="form-group form-group-2" style="display: flex;justify-content: center;align-items: center;">
                                                    <input name="Dosage" v-model="addedProduct.Dosage" autocomplete="off" type="number" id="dosage" style="min-width: 50px;"
                                                    min="1" placeholder="Quantity" class="tBox tBoxSize02">
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div class="form-group form-group-2">
                                                    <select class="browser-default custom-select" v-model="addedProduct.Instruction" name="Instruction">
                                            ...
                                            <td></td>
                                            <td>
                                                <button v-if="productCode.Pack && addedProduct.Description != ''" @click="savePackProduct()" class="btn btn-primary table-icon">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-column">
                                        <div class="form-group form-group-2">
                                            ...
                                            <input name="Code" v-model="productCodeEdit.Code" autocomplete="off" type="number" id="value" placeholder="Code" class="form-control">
                                        </div>
                                        <small style="margin-top: -10px; margin-bottom: 10px; padding-right: 23px;" for="code">
                            ...
                            <div class="pxp-form address-form" v-else-if="type == 'product' && editing">
                                <div class="form-row">
                                    ...
                                    <div class="form-column">
                                        ...
                                        <small style="margin-top: -10px; margin-bottom: 10px; padding-right: 23px;" for="code">CAUTION : changing this will affect pricing</small>
                                    </div>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-column">
                                        <div class="form-group form-group-2" v-if="!productCodeId">
                                            <label for="Price">Price</label>
                                            <input v-model="productCodePrice" type="text" id="Price" class="form-control mb-3" placeholder="Price">
                                            <div v-if="errors.Price" class="invalid-feedback d-block">{{ errors.Price[0] }}</div>
                                        </div>
                            ...
                            <div class="row">
                                <!-- ALTERNATIVE NAMES -->
                                <div class="col-lg-12 wow fadeIn" v-if="productCodeId && !pricingId && !editing && alternativeNames.length > 0">
                                    <h3 style="text-align: center;margin-top: 30px;padding-bottom: 5px;border-bottom: 3px solid #b097ff;float:left;width: 100%;">Alternative Names/Aliases</h3> 
                                    <table class="table table-hover" style="border-bottom: none;">
                                        ...
                                        <tbody>
                                            <tr v-for="name in alternativeNames" :key="name.ProductNameAlternativeID">
                                                ...
                                                <td>
                                                    <button @click="removeAlternativeName(name.ProductNameAlternativeID)" v-if="userInfo.role > 4" title="Remove alternative product name" class="btn btn-primary table-icon">
                                                        <i class="fa fa-trash" style="color: white;"></i>
                                                    </button>
                                                </td>
                                            </tr>
                                ...
                                <!-- PRODUCT LOGS -->
                                <div class="col-lg-12 wow fadeIn" v-if="productCodeId && !pricingId && !editing && logs.length > 0">
                                    <h3 style="text-align: center;margin-top: 30px;padding-bottom: 5px;border-bottom: 3px solid #77c187;float:left;width: 100%;">Product Logs</h3> 
                                    <table class="table table-hover" style="border-bottom: none;">
                                        <thead class="primary-color text-white">
                                            <th>Action</th>
                                            <th>User</th>
                                ...
                                <div class="col-lg-12 wow fadeIn" v-if="packs.length > 0 && !pricingId && !editing">
                                    <h3 style="text-align: center;margin-top: 30px;padding-bottom: 5px;border-bottom: 3px solid #77c187;float:left;width: 100%;">This product is part of the following packaged products</h3> 
                                    <table class="table table-hover" style="border-bottom: none;">
                                        <thead class="primary-color text-white">
                                            <th>Code</th>
                                            <th>Name</th>
                                        ...
                                        <tbody>
                                            <tr v-for="pack in packs" :key="pack.ProductCodeID">
                                                ...
                                                <td><a target="_blank" :href="`/#/products?code=${pack.ProductCodeID}`">View (New Tab)</a></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button v-if="!editing && productCodeId && !productCode.Pack && userInfo.role > 4" :disabled="saving" @click="addPricing()" type="button" class="btn btnSize02 secondaryBtn">
                        Add New Pricing
                    </button>
                    ...
                    <button v-if="editing && productCodeId && userInfo.role > 4" :disabled="saving" @click="back(productCode.Type)" type="button" class="btn btnSize02 secondaryBtn">
                        Back
                    </button>
