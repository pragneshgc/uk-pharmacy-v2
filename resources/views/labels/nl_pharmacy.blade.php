        ...
        @for ($i = 1; $i <= $product->Pages; $i++)
            <div style="width: 315px; height: 140px; display: block;">
                <div style="position: absolute; top: 0;">
                    ...
                    <div class="product-instructions"
                        style="font-size: 11px; padding: 0px 5px; line-height:9px; font-weight: 700; margin-top: 2px;">
                        {{ $product->Instructions }}
                    </div>
                    @if (count($product->WarningLabels) > 0)
                        <div class="product-instructions"
                            style="font-size: 11px; padding: 0px 5px; line-height:9px; font-weight: 700;">
                ...
                @if ($product->InstructionLength < 370)
                    <div class="patient-info"
                        style="display: block; font-size: 11px; padding: 0px 5px; height: 20px; position: absolute; bottom: 42px; font-weight: 700;">
                        ...
                        <div style="float: right; padding-right: 8px; position: relative;">
                            ...
                            margin-right: 10px; @endif">
                                {{ $i }} of {{ $product->Pages + ($product->ExtraPageQuantity > 0 ? 1 : 0) }}
                            </b>
                            <b>{{ date('d/m/Y') }}</b>
                        </div>
                    </div>
                    <div class="additional-information"
                        ...
                        <div
                            style="/* width: 150px; */font-size: 9px;display: inline-block; position: relative;margin-top: auto;white-space: normal;flex-grow: 1;">
                            <b style="position: absolute; left: 60px; top: -2px;">Keep away from children</b>
                            <br>
                            <b>{{ $pharmacy->CompanyName }}, {{ $pharmacy->Address1 }}, {{ $pharmacy->Address3 }}
                                <br> {{ $pharmacy->Address4 }},{{ $pharmacy->Postcode }}, {{ $pharmacy->CodeName }}
                                {{ $pharmacy->Telephone }}</b>
                        </div>
            ...
            @if ($product->InstructionLength >= 370)
                <div style="width: 315px; height: 140px; display: block; position: relative;">
                    <div class="patient-info"
                        style="display: block; font-size: 11px; padding: 0px 5px; height: 20px; position: absolute; top: 10px; font-weight: 700;">
                        <div style="float: left;">
                            <span>{{ $prescription->Title }} {{ $prescription->Name }}
                                {{ $prescription->Surname }}</span>
                        </div>
                        <div style="float: right; padding-right: 8px;">
                            <b
                                style="margin-right: 10px;
                            @if (strlen($prescription->Title . ' ' . $prescription->Name . ' ' . $prescription->Surname) > 37) position: absolute; left: -60px; top: 13px; @endif">
                                {{ $i }} of
        ...
        @if ($product->ExtraPageQuantity > 0)
            <div style="width: 315px; height: 140px; display: block;">
                <div style="position: absolute; top: 0;">
                    <div class="product-name"
                        style="display: block; font-size: 14px; text-align: center; margin-top: -3px; @if (strlen($product->Name . ' ' . $product->DisplayedQuantity . ' ' . $product->Units) > 45) font-size: 12.5px; @endif">
                        <b>{{ $product->Name }} {{ $product->ExtraPageQuantity }} {{ $product->Units }}</b>
                    </div>
                    <div class="product-instructions"
                        style="font-size: 11px; padding: 0px 5px; line-height:9px; font-weight: 700; margin-top: 2px;">
                        {{ $product->Instructions }}
        ...
        <div style="display: block;">
            <div style="padding: 5px; width: 150px; box-sizing: border-box; margin-left: 50px; position: relative;">
                <img style="position: absolute; top: 5px; left: 10px;"
                    ...
                    alt="barcode" />
                <div
                    style="display:block; text-align: center; padding:0; margin: 0; font-size: 20px; margin-top: 10px; position: absolute; left: -50px; top: 75px; width: 315px;">
                    <b style="vertical-align: middle; display: block;"><span
                            style="font-size: 20px;">{{ $prescription->PrescriptionID }}</span></b>
                </div>
            </div>
        </div>
        <div
            style="text-align: center; display: block; margin: 0; padding: 0; clear: both; position: absolute; bottom: 10px;">
            <div style="margin:0;font-size: 14px;"><b><span class="capitalize">{{ $prescription->Name }}</span> <span
                        class="capitalize">{{ $prescription->Surname }}</span></b></div>
            <div style="margin:0;font-size: 11px;"><b>{{ $prescription->DAddress1 }} {{ $prescription->DAddress3 }}
